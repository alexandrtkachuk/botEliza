...
sub command_interface {
	...
	while (1) {
		...
		#add me 
		$user_input= decode('utf8',$user_input);
		#$user_input= encode('utf8',$user_input);
...
sub preprocess {
	...
	WORD: for ($i = 0; $i < @wordsin; $i++) {
		foreach $keyword (keys %{ $self->{pre} }) {
			#print $wordsin[$i], '=', $keyword, "\n";
			#if ($wordsin[$i] =~ /\b$keyword\b/i ) {
			if ($wordsin[$i] eq $keyword ) {
...
sub _testquit {
	...
	#print "\n stirngQuit = $string",length  $string, "!\n" ;

	my ($quitword, @wordsin);

	foreach $quitword (@{ $self->{quit} }) {
		#print $quitword,' - ', length  $quitword,"\n";
		
		#$quitword= decode('utf8',$quitword);
		
		return 1 if ($string =~ /\b$quitword\b/i ) ;
		#return 1 if ($string eq $quitword ) ;
	}
}
